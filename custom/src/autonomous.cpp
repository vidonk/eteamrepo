...
void leftandmid(){
 intake1.spin(fwd,12,volt);
 moveToPoint(-12,31.5,1,1500,false,4); //first stack, 31.5 to 29
 scraper.set(true);
 wait(100,msec);
 driveTo(-4,500,false,8);
 //boomerang(-10,10,-1,-135,0.1,1500,false,8); //align with mid goal, y from 20to 10, -10 x to -14
 turnToAngle(-113, 800,false,6);
 driveChassis(-4,-4);
//drive to mid goal
midgoal.set(false);
 ...
 wait(2000,msec); 
 intake1.spin(reverse, 12, volt);
 wait(1000,msec); // before we get to the midgoal, outtake the top stage, when you getg to the midgoal outtake both for one second and then score 
 intake1.spin(fwd, 12, volt);
 intake1.spin(fwd,-5,volt);
 wait(150,msec);
 driveChassis(0,0);
 midgoal.set(true);
 intake1.spin(fwd, 12, volt);
 wait(1500, msec);
 moveToPoint(-16,0,1,2000,false,9); //drive to matchload, -20 to -10
midgoal.set(false);
 turnToAngle(181,500,true,4);
 wait(100,msec);
 scraper.set(true);
 wait(100, msec);
 driveTo(5,1250,false,5); //drive into matchload
 wait(1500,msec);
 moveToPoint(-16,8,-1,1000,false,7); //align with long goal
 //turnToAngle(173,9000,false,4);
 driveTo(-12,1000,false,11); //drive into long goal
 scraper.set(false);
 intake2.spin(fwd,12,volt);
 wait(1000, msec);
 intake2.spin(fwd, 0, volt);
...
void autonskillsCOMPLEX(){ // get balls align to loader matchload go to other side 
intake1.spin(fwd,12,volt);
 turnToPoint(10, 33.5, 1, 260); //first stack, increased x coord to 9.5, 250 to 275 to 260, x coord to 10.5 to 10
 driveTo(12,2100,false,3); //distance before matchloader is put down, 13.7 to 14, then to 14.5, then to 15, then to 14, 2000 to 2100, 4 to 3.7, 13.7 to 12
 scraper.set(true); //scraper down
 wait(500,msec); // 400 to 500
 driveTo(2,500,false,4);
 moveToPoint(43.5,1,1,1500,false,9); //drive to matchload
 turnToPoint(44.2,-10,1,500);
 intake1.spin(reverse, 12, volt);
 wait(300, msec);
 intake1.spin(fwd, 12, volt);
 wait(200,msec); //added
 driveTo(15.2,1700,true,4); //drive into matchload, 14.6 changed to 14.8, then 14.7, changed to 14.5, then to 15, max voltage decreased, then to 14
 //driveChassis(30, 30); //aaron shen magic
 wait(200,msec); //wait from 270 to 235
intake1.spin(reverse, 12, volt);
intake1.spin(fwd,12,volt);
 wait(175,msec); //added
 moveToPoint(44.5,11,-1,1100,true,9); //back out of matchload, was 45.2 but changed to 45, then changed to 44, then 44.5, then 45 and 1100, then 43 to move more left from 45
 driveTo(-6, 500,true,11); // -7 to -10
 intake1.spin(reverse,12,volt);
 wait(300,msec);
 intake1.spin(fwd,12,volt);
 intake2.spin(fwd,12,volt); //score into long goal
